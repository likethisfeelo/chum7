/** 레거시 backend/shared/lib/quest-submit-validation.ts 이식 (동작 변경 금지). */

export function validateQuestSubmissionContent(
  quest: any,
  content: any,
...
/** 퀘스트 허용 인증 방식 (신규 allowedVerificationTypes, 구버전 verificationType 호환 — 레거시 submit 추출) */
export function resolveQuestAllowedTypes(quest: any): string[] {
  return Array.isArray(quest.allowedVerificationTypes) && quest.allowedVerificationTypes.length > 0
