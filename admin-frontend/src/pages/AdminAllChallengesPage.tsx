...
export const AdminAllChallengesPage = () => {
  ...
  const { data: challengesData, isLoading, error, refetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await apiClient.get(`/adm/challenges/all${query}`);
  ...
  const handleSaveVerificationTypes = async () => {
    ...
    try {
      await apiClient.put(`/adm/challenges/${selectedChallengeId}`, {
    ...
    <div className="max-w-5xl mx-auto p-6 space-y-5">
      ...
      <div className="bg-red-50 border border-red-200 rounded-2xl p-4 space-y-2">
        ...
        <ul className="list-disc pl-5 text-sm text-red-800 space-y-1">
          ...
          <li>백엔드 연결 필수: <code className="font-mono">GET /adm/challenges/all</code> 라우트가 API Gateway에 매핑되어야 합니다.</li>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-4">
        ...
        <SearchableSelect
          options={(challengesData ?? []).map((c: any) => ({
            value: c.challengeId,
