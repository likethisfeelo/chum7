...
export const FeedPage = () => {
  ...
  const currentAlias = useMemo(
    () => ANONYMOUS_NAMES[new Date().getMonth() % ANONYMOUS_NAMES.length],
    []
  );

  const nextApplyDate = useMemo(
    () => format(nextMonday(new Date()), 'M월 d일(E)', { locale: ko }),
    []
  );
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 z-10">
        ...
        <p className="text-sm text-gray-500">1·5·6번 기획 우선 구현 + 페이즈별 확장 준비</p>
      </div>

      <div className="px-6 py-6 space-y-4">
        <section className="bg-white border border-gray-200 rounded-2xl p-4">
          <h2 className="text-sm font-bold text-gray-900">최적 개발안 (우선안)</h2>
          <p className="text-xs text-gray-600 mt-2">
            Phase 1에서 반익명 설정 + 리더 모집 카드 + 광장 기본 탐색을 먼저 제공하고,
            Phase 2에서 진행중 카드/마당 변환/추천 알고리즘을 연결하는 방식이 가장 안전합니다.
          </p>
        </section>
        ...
        <section className="bg-white border border-gray-200 rounded-2xl p-4">
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <p className="text-xs text-gray-600 mt-1">변경 즉시 저장되고, 다음 주부터 새 설정이 적용됩니다.</p>
            </div>
            <button
              type="button"
          ...
          <div className="mt-3 text-xs text-gray-600 space-y-1">
            ...
            <p>• 다음 적용일: {nextApplyDate} (진행 중 챌린지는 기존 설정 유지)</p>
          </div>
        </section>
        ...
        <section className="bg-white border border-gray-200 rounded-2xl p-4">
          ...
          <ul className="text-xs text-gray-600 space-y-1">
            <li>• 공개 리더 챌린지: 리더 브랜드 중심 모집 + 👑 식별</li>
            <li>• 익명 리더 챌린지: 동물 아이콘 기반 동등 소통</li>
            <li>• Phase 3에서 유료 챌린지/수익화 연결</li>
          </ul>
        </section>
            ...
            {showRecords && (
              <>
                {isLoading ? (
                  ...
                  <EmptyState icon="🌌" title="아직 공개 기록이 없어요" description="다음날 변환되는 마당 게시물이 여기에 표시됩니다." />
                ) : (
                  publicRecords.slice(0, 6).map((record: any) => (
                    <article key={record.verificationId} className="border border-gray-200 rounded-2xl p-4 bg-emerald-50/40">
                      ...
                      <p className="text-xs text-gray-500 mt-1">
                        작성자: 비공개 · {format(new Date(record.createdAt), 'M월 d일 HH:mm', { locale: ko })}
                      </p>
                      {record.imageUrl && (
