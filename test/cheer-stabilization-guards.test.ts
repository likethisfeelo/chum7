...
describe('cheer stabilization guards', () => {
  test('thank-message handler uses isThankScoreGranted for eligibility check', () => {
    const src = read('backend/services/cheer/thank-message/index.ts');
    expect(src).toContain('isThankScoreGranted === true');
    expect(src).toContain('isThankScoreGranted = :true');
    expect(src).toContain('MISSING_MESSAGE');
    expect(src).toContain('감사 메시지가 도착했어요');
    expect(src).toContain('NO_VALID_CHEERS');
  });

  test('verification submit grants thank score via grantThankScoreForReceivedCheers', () => {
    const src = read('backend/services/verification/submit/index.ts');
    expect(src).toContain('grantThankScoreForReceivedCheers');
    expect(src).toContain('isThankScoreGranted = :true');
    expect(src).toContain('thankScoreGrantedAt');
    expect(src).toContain('eligibleCheerIds');
    expect(src).toContain('당신을 응원합니다');
  });
  ...
  test('get-profile does not query ticket table', () => {
    const src = read('backend/services/auth/get-profile/index.ts');
    expect(src).not.toContain('USER_CHEER_TICKETS_TABLE');
    expect(src).not.toContain('userId-status-index');
    expect(src).toContain('userId: user.userId');
    expect(src).toContain('stats: user.stats');
  });

  test('infra cheer-stack no longer has thank or use-ticket routes', () => {
    const stack = read('infra/stacks/cheer-stack.ts');
    expect(stack).not.toContain("path: '/cheers/{cheerId}/thank'");
    expect(stack).not.toContain("path: '/cheer/use-ticket'");
    expect(stack).not.toContain('USER_CHEER_TICKETS_TABLE');
    expect(stack).toContain("path: '/cheers/thank-message'");
    expect(stack).toContain("path: '/cheer/my-cheers'");
  });
