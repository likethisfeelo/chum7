...
| ME/Today/Profile 버킷 통일 | 부분 진행 | 화면별 분기 로직 재분화 가능성 | 공통 유틸 완전 단일화 |
...
| 항목 | 현재 상태 | 의존성 | 다음 액션 |
|---|---|---|---|
| 어드민 콘솔 라우팅 완성 | 일부 미연결 | API/권한/운영 정책 | CRUD/review/stats 일괄 연결 |
| 결제/환불 플로우 | 미완 | 외부 PG | free-flow vs PG 연동 의사결정 |
| Extra 인증 공개 전환 UX | 미완 | 프론트 정책 합의 | 공개 설정 UX/노출 규칙 확정 |
