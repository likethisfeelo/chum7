...
- [ ] 친구 요청 알림 → `/friends` 딥링크.
- [ ] 아카이브 타임라인 "더 보기" 페이지네이션(API 커서는 있음).
- [ ] 마당 CommentSection 삭제/배지 레이아웃, 추천 카피, 리더 모드 안내.
- [ ] 프로필에 "친구" 배지(응답 `isFriend` 사용).
- [ ] 로딩/빈 상태 스켈레톤 통일.

### 6.3 기능 후속
- [ ] **친구 알림 볼륨**: 친구 반응 폭주 시 묶음발송 튜닝(기존 bundle 규칙 확인).
- [ ] **단계 E 프론트**: 글로벌 플래그 켤 때 원본 딥링크(원본 보기) 버튼.
- [ ] **친구 요청 배지**: 마이 "친구" 버튼에 받은 신청 수 배지.
- [ ] **차단·해제 시** 관계 아카이브/추천 정리(부분 구현 — 재점검).
- [ ] bulletin 댓글 등 기타 표면의 활동명 스냅샷(현재 생략).

### 6.4 견고성·정합성
- [ ] 친구/아카이브 **라우트 통합테스트**(현재 순수 도메인 테스트만).
