...
- **별도 작업이 필요한 경우(현재 범위 밖)**: *오프라인에서도 앱이 동작*하거나 *네트워크 없이 저장했다가
  나중에 동기화*가 필요하면 → 로컬 캐시 + 동기화 로직을 추가로 설계해야 한다 `[확정 필요: 오프라인 지원 여부]`.
...
- [ ] **오프라인 지원**: 온라인 전용(기본) / 오프라인 캐시+동기화 필요 → §0.5 참고, 필요 시 별도 설계
