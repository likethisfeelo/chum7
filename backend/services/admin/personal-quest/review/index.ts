...
export const handler = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
  try {
    ...
    try {
      input = schema.parse(JSON.parse(event.body || '{}'));
    } catch (parseErr: any) {
      if (parseErr instanceof z.ZodError) return res(400, { error: 'VALIDATION_ERROR', details: parseErr.errors });
      return res(400, { error: 'INVALID_BODY', message: '요청 본문이 올바르지 않습니다' });
    }
    ...
    console.error('Personal quest proposal review error:', error);
