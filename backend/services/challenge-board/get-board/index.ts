...
import { response, getUserId, wasParticipant, isLeader, trackKpiEvent } from '../_shared/common';
...
export const handler = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
  try {
    ...
    const [participant, leader] = await Promise.all([
      wasParticipant(client, challengeId, userId),
      isLeader(client, challengeId, userId),
    ]);
